...
這組規則把可寫入的範圍限制在這幾個路徑，並檢查欄位與長度。`members` 是「跳棋」功能用的（見下面說明），文件 ID 就是使用者自己取的名字。`memberLogs` 是跳棋的除錯記錄（見下面「跳棋同步記錄」說明），只能新增、不能修改或刪除。`lists` 集合是舊版「分享清單」功能留下的，目前網站已經不會再寫入，保留規則只是不影響舊資料，不影響新功能。
...
會做這個是因為：跳棋沒有登入機制，文件 ID 就是名字本身，如果有兩個
不同的人（或同一個人在不同裝置上）不小心打了同一個名字，後寫入的
那筆會直接覆蓋前面的資料，看起來就會像是「選點突然不見了」。有這份
記錄之後，只要看時間軸就能判斷：是自己在某個時間點清空了選取（正常
現象，清單自然會顯示 0 個），還是有別的裝置代號用同一個名字寫入了
空清單（代表名字被別人不小心重複用了，請那個人改用別的名字）。
